          ...
          {posts.length > 0 ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {posts.map((post) => (
                <Link
                  key={post.id}
                  to="/lets-get-real/$slug"
                  params={{ slug: post.slug }}
                  className="group flex flex-col overflow-hidden rounded-3xl bg-cream shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                >
                  {post.cover_url ? (
                    <img
                      src={post.cover_url}
                  ...
                  <div className="flex flex-1 flex-col p-8">
                    ...
                    <h3 className="mt-4 font-serif text-2xl text-evergreen group-hover:underline">
                      {post.title}
                    </h3>
                    <p className="mt-3 flex-1 text-sm leading-relaxed text-forest">
                      {post.excerpt}
                    </p>
                    <span className="mt-5 text-xs uppercase tracking-[0.18em] text-honey">
                      Read more →
                    </span>
                  </div>
                </Link>
            ...
            <>
              <p className="mb-8 text-center text-xs uppercase tracking-[0.22em] text-forest/60">
              ...
              <div className="grid gap-6 md:grid-cols-3">
                {comingSoon.map((post) => (
                  <article
                    key={post.title}
                    className="flex flex-col rounded-3xl bg-cream p-8 shadow-sm"
                  >
                    <span className="font-display text-[0.65rem] uppercase tracking-[0.28em] text-honey">
                      {post.tag}
                    </span>
